...
{
    internal class DepositCalculator : ICalculator
    {
        ...
        private void CalculateResponseParams()
        {
            ...
            while (Duration > 0)
            {
                ...
                if (CheckEndOfYears(tmpDate) && AccuredInterestForReportingPeriod > 0)
                {
                    Tax += TaxCalculation();
        ...
        /// <returns></returns>
        private static int GetDuration(DateTime begin, int term, Constants.Constants.CreditFrequency timeUnit)
        {
            int duration = 0;
            DateTime end;
            switch (timeUnit)
            {
                case Constants.Constants.CreditFrequency.Days:
                    duration = term;
                    break;

                case Constants.Constants.CreditFrequency.Months:
                    end = begin.AddMonths(term);
                    duration = (end - begin).Days;
                    break;

                case Constants.Constants.CreditFrequency.Years:
        ...
        {
            return endPeriod.Year - Start.Year > 0;
        }
